...
func (p *Publisher) Validate(ctx context.Context) error {
	...
	for _, partition := range partitions {
		...
		if isLoopbackHost(leaderHost) {
			return fmt.Errorf("kafka broker metadata advertises loopback listener %s:%d for topic=%s partition=%d; fix Kafka server KAFKA_PUBLIC_HOST", leaderHost, partition.Leader.Port, partition.Topic, partition.ID)
		}
	}
...
func (p *Publisher) writer() *kafka.Writer {
	...
	if strings.TrimSpace(p.Cfg.KafkaClientID) != "" || strings.TrimSpace(p.Cfg.KafkaUsername) != "" {
		transport := &kafka.Transport{ClientID: p.Cfg.KafkaClientID}
		if strings.TrimSpace(p.Cfg.KafkaUsername) != "" || strings.TrimSpace(p.Cfg.KafkaPassword) != "" {

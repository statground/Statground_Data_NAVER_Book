/*
Extended local migration from old ClickHouse objects to new Data_Book_NAVER_* schema.

Run after:
  01_create_data_book_naver_on_cluster.sql
  02_migrate_old_statground_book.sql

This LOCAL script assumes the old databases are visible from the same ClickHouse connection:
...
If the old ClickHouse is a different server/connection, use:
  05b_migrate_old_book_extended_remote_template.sql
*/

SELECT
    'old statground_book.backup_v_book_list_all' AS target,
    count() AS rows,
    uniqExact(uuid) AS uniq_uuid,
    min(created_at) AS min_created_at,
    max(created_at) AS max_created_at
FROM `statground_book`.backup_v_book_list_all;
...
SELECT
    ...
    'statground_book.backup_v_book_list_all' AS source_table
FROM `statground_book`.backup_v_book_list_all
WHERE uuid NOT IN (
    SELECT uuid
    FROM `Data_Book_NAVER_Raw`.legacy_book_catalog
...
SELECT
    'old statground_book.backup_marketplace_url' AS target,
    count() AS rows,
    uniqExact(uuid) AS uniq_uuid,
    uniqExact(uuid_book) AS uniq_book_uuid
FROM `statground_book`.backup_marketplace_url;
...
SELECT
    ...
    type,
    now64(3, 'Asia/Seoul') AS migrated_at,
    'statground_book.backup_marketplace_url' AS source_table
FROM `statground_book`.backup_marketplace_url
WHERE uuid NOT IN (
    SELECT uuid
    FROM `Data_Book_NAVER_Raw`.book_marketplace_url
...
SELECT
    'old statground_book.raw_aladin_publisher_cache' AS target,
    count() AS rows,
    uniqExact(publisher) AS uniq_publisher,
    max(detected_last_page) AS max_detected_last_page
FROM `statground_book`.raw_aladin_publisher_cache;
...
SELECT
    'old log.log_shotalk_search' AS target,
    count() AS rows,
    uniqExact(uuid) AS uniq_uuid,
    uniqExact(commission_link) AS uniq_commission_link
FROM `log`.log_shotalk_search;
...
SELECT
    ...
    commission_link,
    version
FROM `log`.log_shotalk_search FINAL
WHERE uuid NOT IN (
    SELECT uuid
    FROM `Data_Book_NAVER_Raw`.shotalk_search_result
);

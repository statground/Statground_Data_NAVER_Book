/*
Optional transition copy from previously-created Data_Book_* databases to Data_Book_NAVER_*.

Run order when v3 Data_Book_* already contains data:
...
Notes:
- This script does NOT copy Data_Book_Mart directly.
...
SELECT
    'old Data_Book_Log.book_events before copy' AS target,
    count() AS rows
FROM `Data_Book_Log`.book_events;
...
SELECT
    'old Data_Book_Raw.naver_book_raw before copy' AS target,
    count() AS rows
FROM `Data_Book_Raw`.naver_book_raw;
...
SELECT
    'old Data_Book_Log.naver_collect_log before copy' AS target,
    count() AS rows
FROM `Data_Book_Log`.naver_collect_log;
...
SELECT
    'old Data_Book_Raw.naver_commission_link_shotalk before copy' AS target,
    count() AS rows
FROM `Data_Book_Raw`.naver_commission_link_shotalk;
...
SELECT
    'old Data_Book_Raw.legacy_book_catalog before copy' AS target,
    count() AS rows
FROM `Data_Book_Raw`.legacy_book_catalog;
...
SELECT
    'old Data_Book_Raw.book_marketplace_url before copy' AS target,
    count() AS rows
FROM `Data_Book_Raw`.book_marketplace_url;
...
SELECT
    'old Data_Book_Log.aladin_publisher_cache before copy' AS target,
    count() AS rows
FROM `Data_Book_Log`.aladin_publisher_cache;
...
SELECT
    'old Data_Book_Raw.shotalk_search_result before copy' AS target,
    count() AS rows
FROM `Data_Book_Raw`.shotalk_search_result;
